...
df_counts <- read.table("TFM_12O/02.Counts/mageckcount/all_samples_PatuS.count.txt", header=TRUE, sep="\t", check.names=FALSE)

# Asegúrate de que las columnas de counts son numéricas
...
# Renombra a 'counts' para mantener coherencia con el resto del script
counts <- df_counts
...
colnames(counts)[1:2] <- c("sgRNA", "gene")  # Asegura nombres correctos
...
CNVLibrary_norm_patuS <- ccr.GWclean(
  ...
  label = "CNVLibrary_norm_patuS_mageck",
  display = TRUE,
...
write.table(corrected_counts, file = "csv2/PatuS_mageck_CNVcorrected.tsv", sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE, dec = ".")