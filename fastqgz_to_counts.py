...
def seqFileToCounts(infileName, fastaFileName, countFileName, libraryFasta, startIndex=None, stopIndex=None, test=False):
    ...
    seqToIdDict, idsToReadcountDict, _ = parseLibraryFasta(libraryFasta)
    ...
    with open(fastaFileName, 'w') as unalignedFile:
        for i, fastqLine in enumerate(infile):
            ...
            seq = fastqLine.strip()  # Usa la secuencia completa, sin trimming

            aligned = False
            for sgRNA, sgRNA_ids in seqToIdDict.items():
                if sgRNA in seq:  # Verifica si el sgRNA es una subsecuencia de la read
                    for sgRNA_id in sgRNA_ids:
                        idsToReadcountDict[sgRNA_id] += 1
                    numAligning += 1
                    aligned = True
                    break  # Una vez que encuentres un sgRNA, puedes detener la búsqueda

            if not aligned:  # Si no se alinea, guarda la read en el archivo de reads no alineadas
                unalignedFile.write('>%d\n%s\n' % (i, seq))

            curRead += 1

            # Permitir pruebas con un número limitado de lecturas
